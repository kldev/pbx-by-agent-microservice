...
public class RatesEndpointTests : BaseRateTest, IAsyncLifetime
{
    ...
    public async Task GetList_ReturnsRates()
    {
        ...
        // Act
        var response = await _client.PostAsJsonAsync("/api/rates/rates/list", filter);
    ...
    public async Task GetByGid_ExistingRate_ReturnsRate()
    {
        ...
        var response = await _client.GetAsync(
            $"/api/rates/rates/{RateServiceTestDataSeeder.TestRateGid}");
    ...
    {
        // Act
        var response = await _client.GetAsync("/api/rates/rates/non-existing-gid");
    ...
    public async Task Create_ValidRequest_ReturnsCreatedRate()
    {
        ...
        // Act
        var response = await _client.PostAsJsonAsync("/api/rates/rates", request);
    ...
    public async Task LookupRate_ExistingPrefix_ReturnsMatchingRate()
    {
        // Act
        var response = await _client.GetAsync(
            $"/api/rates/rates/lookup?tariffGid={RateServiceTestDataSeeder.TestTariffGid}&phoneNumber=+48123456789");
    ...
    public async Task LookupRate_NonMatchingPrefix_Returns404()
    {
        // Act
        var response = await _client.GetAsync(
            $"/api/rates/rates/lookup?tariffGid={RateServiceTestDataSeeder.TestTariffGid}&phoneNumber=+33123456789");
    ...
    public async Task Update_ValidRequest_ReturnsUpdatedRate()
    {
        ...
        var response = await _client.PutAsJsonAsync(
            $"/api/rates/rates/{RateServiceTestDataSeeder.TestRateGid}", updateRequest);
    ...
    public async Task Delete_ExistingRate_ReturnsSuccess()
    {
        ...
        var createResponse = await _client.PostAsJsonAsync("/api/rates/rates", createRequest);
        var created = await createResponse.Content.ReadFromJsonAsync<RateResponse>();

        // Act
        var response = await _client.DeleteAsync($"/api/rates/rates/{created!.Gid}");
