using System.Security.Claims;
using App.Bps.Enum;
using App.Shared.Web.ApiResultPattern;
using App.Shared.Web.BaseModel;
using Microsoft.AspNetCore.Mvc;
using RateService.Api.Features.Rates.Model;

namespace RateService.Api.Features.Rates;

public static class Endpoint
{
    private static readonly AppRole[] AllRoles = [AppRole.Root, AppRole.Ops];

    public static void MapRateEndpoints(this IEndpointRouteBuilder app)
    {
        var group = app.MapGroup("/api/rates/rates")
            .WithTags("Rates")
            .RequireAuthorization();

        // POST /api/rates/rates/list
        group.MapPost("/list", GetList)
            .WithName("GetRatesList")
            .WithSummary("Lista stawek")
            .Produces<PagedResult<RateResponse>>()
            .Produces<ApiErrorResponse>(400);

        // GET /api/rates/rates/{gid}
        group.MapGet("/{gid}", GetByGid)
            .WithName("GetRateByGid")
            .WithSummary("Pobierz stawkę")
            .Produces<RateResponse>()
            .Produces<ApiErrorResponse>(404);

        // POST /api/rates/rates
        group.MapPost("/", Create)
            .WithName("CreateRate")
            .WithSummary("Utwórz stawkę")
            .Produces<RateResponse>()
            .Produces<ApiErrorResponse>(400);

        // PUT /api/rates/rates/{gid}
        group.MapPut("/{gid}", Update)
            .WithName("UpdateRate")
            .WithSummary("Aktualizuj stawkę")
            .Produces<RateResponse>()
            .Produces<ApiErrorResponse>(400)
            .Produces<ApiErrorResponse>(404);

        // DELETE /api/rates/rates/{gid}
        group.MapDelete("/{gid}", Delete)
            .WithName("DeleteRate")
            .WithSummary("Usuń stawkę")
            .Produces<bool>()
            .Produces<ApiErrorResponse>(400)
            .Produces<ApiErrorResponse>(404);

        // GET /api/rates/rates/lookup?tariffGid=xxx&phoneNumber=+48123456789
        group.MapGet("/lookup", Lookup)
            .WithName("LookupRate")
            .WithSummary("Znajdź stawkę dla numeru")
            .Produces<RateResponse>()
            .Produces<ApiErrorResponse>(404);
    }

    private static async Task<IResult> GetList(
        ClaimsPrincipal user,
        IRateService service,
        [FromBody] RateListFilter filter)
    {
        return await EndpointHelpers.HandleAuthRequestAsync(
            user,
            filter,
            async (auth, req) => await service.GetListAsync(auth, req),
            AllRoles);
    }

    private static async Task<IResult> GetByGid(
        ClaimsPrincipal user,
        IRateService service,
        string gid)
    {
        return await EndpointHelpers.HandleAuthRequestAsync(
            user,
            async auth => await service.GetByGidAsync(auth, gid),
            AllRoles);
    }

    private static async Task<IResult> Create(
        ClaimsPrincipal user,
        IRateService service,
        [FromBody] CreateRateRequest request)
    {
        return await EndpointHelpers.HandleAuthRequestAsync(
            user,
            request,
            async (auth, req) => await service.CreateAsync(auth, req),
            AllRoles);
    }

    private static async Task<IResult> Update(
        ClaimsPrincipal user,
        IRateService service,
        string gid,
        [FromBody] UpdateRateRequest request)
    {
        return await EndpointHelpers.HandleAuthRequestAsync(
            user,
            request,
            async (auth, req) => await service.UpdateAsync(auth, gid, req),
            AllRoles);
    }

    private static async Task<IResult> Delete(
        ClaimsPrincipal user,
        IRateService service,
        string gid)
    {
        return await EndpointHelpers.HandleAuthRequestAsync(
            user,
            async auth => await service.DeleteAsync(auth, gid),
            AllRoles);
    }

    private static async Task<IResult> Lookup(
        ClaimsPrincipal user,
        IRateService service,
        [FromQuery] string tariffGid,
        [FromQuery] string phoneNumber)
    {
        return await EndpointHelpers.HandleAuthRequestAsync(
            user,
            async auth => await service.FindRateForNumberAsync(auth, tariffGid, phoneNumber),
            AllRoles);
    }
}
